...
st.title("?? Fantasy Premier League – Vývoj bodù v minilize")

league_id = st.number_input("Zadej ID miniligy (napø. 36264):", min_value=1, value=36264, step=1)
...
if st.button("?? Zobrazit graf"):
    ...
    for entry_id, name in entries:
        try:
            ...
            st.warning(f"Chyba pøi naèítání dat pro {name}: {e}")

    if not df.empty:
        df_cum = df.cumsum()
        st.subheader("?? Vývoj celkových bodù")
